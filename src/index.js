...
class RabbitmqPubSub {
				...
				this._connection
					...
					.then((channel) => {
						return channel
							...
							.then(({ queue }) => {
								return channel.bindQueue(queue, this.exchangeName, 'default-pubsub').then(() => {
									channel
										.consume(
											queue,
											(message) => {
	...
	_bindSubscribeQueue (channelName) {
		return this._connection.getChannel().then((channel) => {
			return channel.bindQueue(this.subscribeQueueName, this.exchangeName, channelName).then(() => {
	...
	_unbindSubscribeQueue (channelName) {
		return this._connection.getChannel().then((channel) => {
			return channel.unbindQueue(this.subscribeQueueName, this.exchangeName, channelName).then(() => {
				return channel.close();
			});
		});
	}
